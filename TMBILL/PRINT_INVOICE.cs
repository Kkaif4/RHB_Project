...
public class PRINT_INVOICE : MetroForm
{
	private string id = "";
	...
	private void PRINT_INVOICE_Load(object sender, EventArgs e)
	{
		...
		try
		{
			...
			try
			{
				val2.Connection = val;
				((DbConnection)(object)val).Open();
				((DbCommand)(object)val2).CommandText = "select * from r_header where id='5'";
		...
		try
		{
			...
			try
			{
				val2.Connection = val;
				((DbConnection)(object)val).Open();
				((DbCommand)(object)val2).CommandText = "select * from r_invoice where id='" + id + "'";
				SqlDataAdapter val3 = new SqlDataAdapter(val2);
		...
		try
		{
			...
			try
			{
				val2.Connection = val;
				((DbConnection)(object)val).Open();
				((DbCommand)(object)val2).CommandText = "select * from r_invoice_detail where invoice_id='" + id + "'";
				SqlDataAdapter val3 = new SqlDataAdapter(val2);
	...
	private void PRINT_INVOICE_FormClosed(object sender, FormClosedEventArgs e)
	{
		try
		{
	...
	private void InitializeComponent()
	{
		...
		ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(PRINT_INVOICE));
		...
		val.Name = "DataSet1";
		val.Value = r_invoiceBindingSource;
		val2.Name = "DataSet2";
		val2.Value = r_invoice_detailBindingSource;
		...
		reportViewer1.LocalReport.ReportEmbeddedResource = "TMBILL.Report_INVOICE.rdlc";
		((Control)reportViewer1).Location = new Point(1, 39);
		((Control)reportViewer1).Name = "reportViewer1";
		((Control)reportViewer1).Size = new Size(437, 309);
		...
		((Form)this).ClientSize = new Size(461, 371);
		((Control)this).Controls.Add((Control)(object)reportViewer1);
		((Form)this).Icon = (Icon)componentResourceManager.GetObject("$this.Icon");
		((Control)this).Name = "PRINT_INVOICE";
		((Form)this).FormClosed += new FormClosedEventHandler(PRINT_INVOICE_FormClosed);
		((Form)this).Load += PRINT_INVOICE_Load;
