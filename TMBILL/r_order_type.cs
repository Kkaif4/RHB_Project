...
public class r_order_type : MetroForm
{
	...
	private void InitializeComponent()
	{
		...
		ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(r_order_type));
		...
		((DataGridViewColumn)Column2).HeaderText = "Order Form";
		...
		metroTextBox5.PromptText = "Enter Order From";
		...
		metroTextBox5.WaterMark = "Enter Order From";
		...
		((Control)this).Name = "r_order_type";
		((MetroForm)this).Resizable = false;
		((Control)this).Text = "Order Type";
		((Form)this).FormClosed += new FormClosedEventHandler(r_order_type_FormClosed);
	...
	private void bindgrid()
	{
		...
		try
		{
			...
			try
			{
				...
				((DbCommand)(object)val2).CommandText = "select * from r_order_from where order_from like '" + ((Control)metroTextBox6).Text + "%'";
	...
	private void SaveFunction()
	{
		...
		if (new USER_PERMISSION().Permission("7", "Insert"))
		{
				...
				try
				{
					...
					try
					{
						...
						try
						{
							val2.Connection = val;
							((DbConnection)(object)val).Open();
							string commandText = "insert into r_order_from(order_from) values('" + ((Control)metroTextBox5).Text + "')";
							((DbCommand)(object)val2).CommandText = commandText;
							((DbCommand)(object)val2).ExecuteNonQuery();
							((DbConnection)(object)val).Close();
							bindgrid();
							ResetFunction();
							if (1 == (int)MetroMessageBox.Show((IWin32Window)(object)this, "Order From Type Added Successfully", "Done", (MessageBoxButtons)0, (MessageBoxIcon)64))
							{
								((Control)this).Focus();
								((Form)this).Activate();
							}
	...
	private void UpdateMethod()
	{
		...
		if (new USER_PERMISSION().Permission("7", "Update"))
		{
			if (gid != "")
			{
				if (((ContainerControl)this).ValidateChildren())
					...
					try
					{
						...
						try
						{
							...
							try
							{
								val2.Connection = val;
								((DbConnection)(object)val).Open();
								string commandText = "update r_order_from set order_from='" + ((Control)metroTextBox5).Text + "' where id='" + gid + "'";
								((DbCommand)(object)val2).CommandText = commandText;
								((DbCommand)(object)val2).ExecuteNonQuery();
								((DbConnection)(object)val).Close();
								bindgrid();
	...
	private void DeleteMethod()
	{
		...
		if (new USER_PERMISSION().Permission("7", "Delete"))
		{
			if (gid != "")
			{
					...
					try
					{
						...
						try
						{
							...
							try
							{
								val2.Connection = val;
								((DbConnection)(object)val).Open();
								string commandText = "delete from r_order_from where id='" + gid + "'";
								((DbCommand)(object)val2).CommandText = commandText;
								((DbCommand)(object)val2).ExecuteNonQuery();
								((DbConnection)(object)val).Close();
								bindgrid();
	...
	private void r_order_type_FormClosed(object sender, FormClosedEventArgs e)
	{
		((Control)Functions.static_mainform).Focus();
		((Form)Functions.static_mainform).Activate();
	}
